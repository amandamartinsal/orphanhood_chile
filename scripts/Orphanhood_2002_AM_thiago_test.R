...
# ip4: sex
# ip5: age
# ip9c: education (it's an open question and has a lot of missing)
# ip9t: education type (it has categories and less missing)
# vp1: type of relationship with mother or tutor
# vp2: Is your mother alive?
# vp3: Mother's age 
# vp5: Mother's age of death
# vp6: Does your mother know read and write?
# vp7c: Mother's educational attainment
# vp7t: Mother's type education
# vp12: Is your father alive?
# vp13: Father's age
# vp15: Father's age of death
# vp16: Does your father know read and write? 
# vp17c: Father's educational attainment
# vp17t: Father's type education
# vp8: Mother's labor status
# vp9: Mother's occupation
# vp10: Mother's labor stability
# vp21: Economic situation the children was born - could be a proxy to the mother's economic situation
# vp22: Actual economic situation
# iip9: Occupation
# ivp3a: Zone : rural or urban
...
eps2002.1 <- read_dta("data/base1.dta") %>%
  select(gse,factor, vp1, vp2, vp3,vp5, vp6, vp7c, vp7t, vp12,
         vp13, vp15, vp16, vp17c, vp17t, folio_n20, ivp3a,
         vp8, vp9, vp10, vp21, vp22) %>%
  ...
  mutate(
    mothers_edu = factor(
      case_when(
        ...
        as.numeric(medutype) %in% 17 ~ 3      # don't know
      ),
      levels = c(1, 2, 3), labels = c("low", "high", "dknow"), ordered = TRUE
    ),
    children_edu = factor(
      case_when(
...
## |- Descriptive (with and without weights) ------------------------------------
...
s_bs %>%
  filter(between(age_group_numeric, 25, 50)) %>% 
  ggplot(aes(y = factor(mothers_edu, levels = c("dknow", "high", "low")), 
  ...
  scale_color_viridis_d(
    ...
    direction = -1,
    labels = c("low" = "Low", "high" = "High", "dknow" = "Unkown")) +
  scale_y_discrete(
    labels = c("dknow" = "Unkown", "high" = "High", "low" = "Low")) +
  labs(x = TeX("Survival probabilities between age 25 and  25 + n ($_{n}p_{25}$)"),
       y = "Mother's Education") +
  coord_cartesian(xlim = c(.4,1)) + 
  facet_wrap(~age_group_numeric) 

ggsave("outputs/np25_2.pdf",
...
## e50 facet by mother's edu, without "don't know" and e60 for total population from WPP
...
# With information from WPP total population
...
ggsave("outputs/e60_WPP2.pdf",
       width = 16, height = 10, units = "cm")
  
# Without information from WPP total population 
...
ggplot(data = s_bs_wpp, aes(x = cohort, group = children_edu, color = children_edu)) +
  ...
  scale_color_manual(
    ...
    values = viridis::viridis_pal(direction = -1)(length(unique(s_bs_wpp$children_edu))),
    labels = c("low" = "Low", "high" = "High")) +   
  scale_x_continuous(breaks = seq(1925, 1950, 5)) +
  scale_y_continuous(breaks = seq(20,50,5)) +
  labs(x = "Mother's Mean Year of Birth", y = TeX("Mother's Life Expectancy at age 60")) + 
  theme(panel.grid.minor.x = element_blank())

ggsave("outputs/e60.pdf",
       width = 16, height = 10, units = "cm")

# With information from WPP total population (changing axis)
...
## Difference in e60 facet by mother's edu, without "don't know"
